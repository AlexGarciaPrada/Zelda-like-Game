...
func _equip_spell(input:String):
	if self.visible:
		var square = get_parent().get_parent()._get_spell_square(int(input))
		get_parent().get_parent()._correct_equipment(_get_element_square_texture_path(show_element,show_num))
